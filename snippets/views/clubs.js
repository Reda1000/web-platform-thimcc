...
const formatterClubs = () => {
  document.querySelector('#clubs div.content').innerHTML =
    clubsView.data
      ...
      .map(
        (_, i) => `
<a role="button" href='javascript:setRouterParam("popup", "clubView_${i}");' id='clubView_${i}' style='text-decoration: none; border: 1px solid #1D71B9; cursor: pointer; flex:1 0 11%; margin:0.25rem; min-width:300px; padding:0.5rem; display: flex; flex-direction: row; align-items: center;'>
  <b>${_.number}</b>&nbsp;|&nbsp;
